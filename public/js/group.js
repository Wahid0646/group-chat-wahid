...
async function createGroup() {
  try {
    ...
    const res = await axios.post(
      "http://13.48.26.59:4000/group/createGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function getGroups() {
  try {
    const token = localStorage.getItem("token");
    const res = await axios.get("http://13.48.26.59:4000/group/getGroups", {
      headers: { Authorization: token },
    });
    groups.innerHTML = "";
...
async function addToGroup() {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.post(
      "http://13.48.26.59:4000/group/addToGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function deleteFromGroup() {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.post(
      "http://13.48.26.59:4000/group/deleteFromGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function groupMembers() {
  try {
    ...
    const res = await axios.get(
      `http://13.48.26.59:4000/group/groupMembers/${groupName}`,
      { headers: { Authorization: token } }
    );
...
function logout() {
  localStorage.clear();
  window.location.href = "http://13.48.26.59:4000";
}
